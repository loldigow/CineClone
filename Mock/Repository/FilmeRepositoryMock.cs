...
namespace Mock.Repository
{
    public class FilmeRepositoryMock : IFilmeRepository
    {
        ...
        {
            await Task.Delay(11);
            var teste = _filmes.FirstOrDefault(x => x.Codigo == codigoFilme);
            return teste;
        }

        public async Task<string> ObtenhaNomeFilme(int filmeCodigo)
        {
            await Task.Delay(1);
            return _filmes.First(x => x.Codigo == filmeCodigo)?.NomeFilme;
        }

        public async Task<List<FilmeModel>> ObtenhaTodosFilmesAtivos()
        {
            await Task.Delay(1000);
            return _filmes.Where(x => x.EmCartaz).ToList();
        }
    }
}
